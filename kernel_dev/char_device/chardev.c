...
#define SUCCESS 0
#define DEVICE_NAME "chardev"	/* Dev name as it apears in /proc/decices */
			   ...
			   loff_t *offset) 
{
	...
	if (!*(msg + *offset)) {	/* we are at the end of message */
	...
	*offset += bytes_read;
	...
	/* Most read functions returns the number of bytes put into the buffer. */
	return bytes_read;
}
