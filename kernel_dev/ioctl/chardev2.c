			   ...
			   loff_t *offset)
{
	...
	int bytes_read = 0;
	/* How far did the process reading the message get? Useful is the
	...
	/* Start point of the accessed message */
	message_ptr += *offset;

	/* Actually put the data int the buffer */
	...
	*offset += bytes_read;
	...
	/* Read functions are supported to return the number of bytes actually
			 ...
			 unsigned long ioctl_param)
{
	...
	/* We don't want to talk to two process at the same time. */
	...
	switch (ioctl_num) {
		case IOCTL_SET_MSG:
			/* Received a pointer to a message (in user space) and set
			 * that to be the device's message. Get the parameter given
		...
		case IOCTL_GET_MSG:
			...
			/* Given the current message to the calling process - the
 ...
 * is kept in the device table, it can't be local to init_module. NULL is
